...
| `description?` | `string` | `-` | - |
| `errorMessage?` | `string \| ((validation: ValidationResult) => string)` | `-` | - |
| `showSwatch?` | `boolean` | `true` | Show a color swatch preview inside the field |
| `swatchContent?` | `React.ReactNode` | `-` | Custom content to replace the default swatch (e.g. a ColorPicker trigger) |
